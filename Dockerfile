...
RUN chown -R squid:squid /etc/squid/conf.d.tail/
RUN chmod +x /etc/squid/conf.d.tail/squid-log.conf
RUN chmod +x /etc/squid/squid.conf
RUN chown -R squid:squid /etc/squid/squid.conf
RUN	set -x && \
